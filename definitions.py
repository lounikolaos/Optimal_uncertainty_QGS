...
def _resolve_q_filter_func(q_filter: str):
    ...
    q_filter == 'f' selects f_metropolis_BA, which is already the full f(nu) (not a q(nu)
...
def f_metropolis_BA(nu: np.ndarray, beta: float, S: float, sigma: float = 2.0) -> np.ndarray:
    nu = np.asarray(nu, dtype=float)
    if beta < 0:
        raise ValueError("beta should be nonnegative.")
    arg = nu + beta * sigma ** 2 / 2
    envelope =  np.exp(-beta * np.maximum(arg, 0.0))
...
def build_L_alpha(coupling_ops: np.ndarray, eigenvalues: np.ndarray, eigenvectors: np.ndarray,
    ...
    if q_filter_func == 'f':
        def f_nu(nu):
            return f_metropolis_BA(nu, beta, S, sigma).astype(complex)
...
def filter_function_nu(nu: np.ndarray, beta: float, S: float, sigma: float, q_filter_func: Callable) -> np.ndarray:
    ...
    q_filter_func == 'f' selects f_metropolis_BA, which is already the full f(nu) and
