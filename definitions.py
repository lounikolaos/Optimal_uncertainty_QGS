...
def _metropolis_gamma_BA(w_d, beta, sigma_E):
    """sqrt of the (smeared) Metropolis transition rate (gibbs_sampler_BA.py convention)."""
    arg = w_d + beta * sigma_E ** 2 / 2
    return np.sqrt(np.exp(-beta * np.maximum(arg, 0.0)))


def _gaussian_gamma_BA(w_d, beta, sigma_E):
    """sqrt of the (smeared) Gaussian transition rate (gibbs_sampler_BA.py convention).

    w_gamma is fixed by convention to 1/beta; sigma_gamma is then solved for from the one
    remaining condition (raises if beta, sigma_E leave no valid sigma_gamma >= 0).
    """
    w_gamma = 1 / beta
...
GAMMAS_BA = {
    "metropolis": _metropolis_gamma_BA,
    "gaussian": _gaussian_gamma_BA,
}


Q_FILTER_FUNCTIONS = {
    "metropolis": q_metropolis_bumped, 
    "gaussian": q_gaussian_bumped,
    "gaussian_bumped_sigma": q_gaussian_bumped_sigma,
    "gaussian_sigma": q_gaussian_sigma,
}


def _resolve_q_filter_func(q_filter: str):
    """Resolves the `q_filter` string into what build_L_alpha/filter_function_nu expect.

    q_filter == 'f' selects f, which is already the full f(nu) (not a q(nu)
    that still needs the exp(-beta*nu/4) KMS prefactor) -- build_L_alpha and
    filter_function_nu special-case this 'f' sentinel to evaluate/Fourier-transform it
    directly instead of going through Q_FILTER_FUNCTIONS.
    """
    if q_filter == 'f_g':
        return 'f_g'
    if q_filter == 'f_m':
            return 'f_m'
    return Q_FILTER_FUNCTIONS[q_filter]
...
def build_L_alpha(coupling_ops: np.ndarray, eigenvalues: np.ndarray, eigenvectors: np.ndarray,
    ...
    if q_filter_func == 'f_g':
        def f_nu(nu):
            return _gaussian_gamma_BA(nu, beta, sigma).astype(complex)
        f_t = _fft_frequency_to_time(grid.nu_grid, grid.dnu, f_nu)                # (n_t,)
    if q_filter_func == 'f_m':
        def f_nu(nu):
            return _metropolis_gamma_BA(nu, beta, sigma).astype(complex)
        f_t = _fft_frequency_to_time(grid.nu_grid, grid.dnu, f_nu)                # (n_t,)
    else:
...
def build_lindbladian_time_domain(hamiltonian: np.ndarray, beta: float, sigma: float, S: float,
    ...
    q_filter_func = _resolve_q_filter_func(q_filter)
...
def filter_function_nu(nu: np.ndarray, beta: float, S: float, sigma: float, q_filter_func: Callable) -> np.ndarray:
    ...
    q_filter_func == 'f' selects f, which is already the full f(nu) and
    needs no extra KMS prefactor.
    """
    if q_filter_func == 'f_g':
        return _gaussian_gamma_BA(nu, beta, sigma).astype(complex)
    if q_filter_func == 'f_m':
...
def make_gaussian_window_normalized(w_d, deltaE, sigma):
    """
    Returns f[k, m, n] such that for each (m, n), sum_k |f[k,m,n]|^2 * dnu = 1 -- i.e. f is
    normalized as a *density* (discretized approximation of a continuous window with unit L2
    norm), not as a discrete unit vector. This makes f converge to a fixed shape as the
    frequency grid (w_d) is refined; normalizing to sum_k |f_k|^2 = 1 instead (no dnu) makes f
    -- and hence the Lindbladian rates built from it in process_jump_operators_all_bohr_fast,
    which separately multiplies by sqrt(dnu) as the Kraus-operator quadrature weight -- scale
    with dnu and never converge as the grid is refined.
    """
    dnu = w_d[1] - w_d[0]
    w = w_d[:, None, None]
    base = np.exp(-((w - deltaE) ** 2) / (4 * sigma ** 2))

    norm2 = np.sum(np.abs(base) ** 2, axis=0) * dnu
    norm = np.sqrt(norm2)
    norm[norm == 0] = 1.0
...
def process_jump_operators_all_bohr_fast(A_list, evals, evecs, deltaE, w_d, beta, sigma_E,
    ...
    `transition_rate` selects the weighting function from `GAMMAS_BA` (currently
    "metropolis" or "gaussian").

    Note: evals/evecs/deltaE must be precomputed once (see get_eig) and passed
    in -- they are NOT recomputed here.
    """
    ...
    weight_fn = GAMMAS_BA[transition_rate]
...
def build_lindbladian_anthony_bohr_2(hamiltonian: np.ndarray, beta: float, sigma: float, S: float,
                                      grid: IntegrationGrid, coupling_ops: np.ndarray,
                                      q_filter: str = "metropolis",
                                      eig_cache: Optional[Dict] = None) -> Tuple[np.ndarray, np.ndarray]:
    """Same as build_lindbladian_anthony_bohr, but X_s is built from the discretized-frequency
    Bohr-energy-basis construction (process_jump_operators_all_bohr_fast, ported from
    gibbs_sampler_BA.py) instead of the analytic Bohr-frequency sum (build_X_s_bohr).

    Despite the `q_filter` name (kept for a uniform call signature across LINDBLADIANS,
    so this construction can be dropped into run_convergence_sweep etc.), the string is looked
    up in `GAMMAS_BA` (currently "metropolis" or "gaussian"), not `Q_FILTER_FUNCTIONS` -- GAMMAS_BA's
    weight functions take (w_d, beta, sigma_E), not (nu, beta, S, sigma). `S` is accepted only
    for that same uniform call signature and is unused.
    """
    eigenvalues, eigenvectors = get_eig(hamiltonian, eig_cache)
    deltaE = eigenvalues[:, None] - eigenvalues[None, :]
    d = hamiltonian.shape[0]

    X_s = process_jump_operators_all_bohr_fast(
        coupling_ops, eigenvalues, eigenvectors, deltaE, grid.nu_grid, beta, sigma,
        transition_rate=q_filter,
    )
    kraus_ops = (X_s).reshape(-1, d, d)
...
''''
The first 4 Lindbladians in LINDBLADIANS are the same one, namely the "Robert Lindbladian". The last on is a differnt Lindbladian, the Lindbladian from my BA.
'''

LINDBLADIANS: Dict[str, Callable] = {
    "L_R_time_domain": build_lindbladian_time_domain, # time-domain construction (page 39)
    "L_R_frequency_domain": build_lindbladian_frequency_domain, # frequency domain construction (page 36)
    "L_R_anthony_time_construction": build_lindbladian_anthony, # We know the transition part (=phi), and then with the X'_s, we can build B and R. Here X' is build in the time domain.
    "L_R_anthony_frequency_construction": build_lindbladian_anthony_bohr, # We know the transition part (=phi), and then with the X'_s, we can build B and R. Here X' is build in the frequency domain.
    "BA": build_lindbladian_anthony_bohr_2, # Lindbladian from my BA thesis
}
...
def grid_with_beta_dependency(beta: float, sigma: float, base_t_max: float = 100.0, base_beta: float = 1.0,
                    base_n_t: int = 1201, min_points_per_sigma_window: int = 8) -> IntegrationGrid:
    """Grow the FFT time domain in proportion to beta, and shrink dt as sigma grows.
...
def run_convergence_sweep(hamiltonian: np.ndarray, sigma_values: Sequence[float], beta_values: Sequence[float],
                           ...
                           constructions: Sequence[str] = ("L_R_time_domain", "L_R_frequency_domain",
    ...
    for i, (construction, sigma, beta) in enumerate(combos):
        ...
        builder = LINDBLADIANS[construction]
        ...
        rho_converged, conv_time, spectral_gap = evolver.evolve_until_converge()
...
CONSTRUCTION_ORDER = ["L_R_time_domain", "L_R_frequency_domain",
                      "L_R_anthony_time_construction", "L_R_anthony_frequency_construction", "BA"]
...
def plot_convergence_lines(df: pd.DataFrame, scale = 'log', constructions: Sequence[str] = CONSTRUCTION_ORDER):
    present = [c for c in constructions if c in set(df["construction"])]
...
def plot_spectral_gap_lines(df: pd.DataFrame, scale = 'log', constructions: Sequence[str] = CONSTRUCTION_ORDER):
    present = [c for c in constructions if c in set(df["construction"])]
    fig, axes = plt.subplots(1, len(present), figsize=(6 * len(present), 5), squeeze=False, sharey=True)
    for ax, construction in zip(axes[0], present):
        sub = df[df["construction"] == construction]
...
def plot_convergence_time_times_hastle(
        df: pd.DataFrame,
        constructions: Union[str, Sequence[str]] = CONSTRUCTION_ORDER,
        hastle_function: Callable[[np.ndarray, np.ndarray], np.ndarray] = hastle_function_inverse_sigma,
        metric: str = "convergence_time",
        hastle_name: Optional[str] = None) -> Dict[str, pd.DataFrame]:
    """weighted vs sigma, one line per beta, one subplot per construction.

    `df` is the raw sweep results DataFrame (as returned by `run_convergence_sweep`), not a
    pre-built weighted table -- the weighting is computed here from `metric` and
    `hastle_function`, so the plotted data always matches its label:

      metric="convergence_time" (default) -- plots `convergence_time_times_hastle(df, ...)`
      metric="inverse_gap"                -- plots `spectral_gap_times_hastle(df, ...)`

    `constructions` may be a single construction name (e.g. "L_R_frequency_domain") to plot just
    that one, or a sequence of names.

    `hastle_name`, if given, is shown in the figure title so it's clear which weighting
    was used to produce the plot.

    Returns the weighted_by_construction dict (same shape as `convergence_time_times_hastle` /
    `spectral_gap_times_hastle`'s return value), in case you want to reuse it, e.g. with
    `plot_normalized_construction_comparison`.
    """
    if isinstance(constructions, str):
        constructions = (constructions,)

    if metric == "convergence_time":
        weighted_by_construction = convergence_time_times_hastle(df, constructions, hastle_function)
        quantity = "convergence time"
    elif metric == "inverse_gap":
        weighted_by_construction = spectral_gap_times_hastle(df, constructions, hastle_function)
        quantity = "inverse spectral gap"
    else:
        raise ValueError(f"Unknown metric={metric!r}; expected 'convergence_time' or 'inverse_gap'.")
    ...
    for ax, construction in zip(axes[0], present):
        ...
        ax.set_xlabel("sigma")
        ax.set_ylabel(f"weighted {quantity}")
        ax.set_title(construction)
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=8)
    title = f"{quantity.capitalize()} times HASTLE w.r.t. sigma, for each beta, by construction"
...
def spectral_gap_times_hastle(
    df: pd.DataFrame,
    constructions: Sequence[str] = CONSTRUCTION_ORDER,
    hastle_function: Callable[[np.ndarray, np.ndarray], np.ndarray] = hastle_function_inverse_sigma,
) -> Dict[str, pd.DataFrame]:
    """Builds, for each construction, a (sigma x beta) table of (1 / spectral_gap) * hastle_function.
    ...
    weighted_by_construction = {}
    for construction, sub in df.groupby('construction'):
        pivot = 1.0 / sub.pivot_table(index='sigma', columns='beta', values='spectral_gap')
        S, B = np.meshgrid(pivot.index.values, pivot.columns.values, indexing='ij')
        f_grid = pd.DataFrame(hastle_function(S, B), index=pivot.index, columns=pivot.columns)
        weighted_by_construction[construction] = pivot * f_grid
...
def plot_spectral_gap_times_hastle(weighted_by_construction: Dict[str, pd.DataFrame],
                         constructions: Sequence[str] = CONSTRUCTION_ORDER,
                         hastle_name: Optional[str] = None):
    """weighted inverse spectral gap vs sigma, one line per beta, one subplot per construction.
    ...
    for ax, construction in zip(axes[0], present):
        ...
        for beta in weighted.columns:
            ax.plot(weighted.index, weighted[beta], marker="o", label=f"beta={beta:.3g}")
        ax.set_xlabel("sigma")
        ax.set_ylabel("weighted inverse spectral gap")
        ax.set_title(construction)
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=8)
    title = "Inverse spectral gap times HASTLE w.r.t. sigma, for each beta, by construction"
    if hastle_name is not None:
        title += f"  (weighting: {hastle_name})"
    fig.suptitle(title)
    plt.tight_layout()
...
def _normalize_series(series: pd.Series, normalize: str) -> pd.Series:
    if normalize == "max":
        ...
        raise ValueError(f"Unknown normalize={normalize!r}; expected one of "
                          f"'max', 'minmax', 'zscore', 'none'.")


def plot_normalized_construction_comparison(
    weighted_by_construction_1: Dict[str, pd.DataFrame],
    weighted_by_construction_2: Dict[str, pd.DataFrame],
    construction_1: str = "L_R_frequency_domain",
    construction_2: str = "BA",
    normalize: str = "max",
    hastle_name: Optional[str] = None,
):
    """Overlay two constructions' weighted-convergence-time curves (one subplot per beta),
    each rescaled by `normalize` so they can be compared on their *shape* alone, independent
    of their different overall scales.

    `weighted_by_construction_1`/`_2` are dicts as returned by `convergence_time_times_hastle`
    (sigma x beta DataFrames, keyed by construction name) -- typically two separate sweeps,
    e.g. one for "L_R_frequency_domain" and one for "BA".

    `normalize` controls how each (construction, beta) curve is rescaled before plotting:
      - "max":    divide by the curve's own max absolute value (peak -> 1)
      - "minmax": rescale to [0, 1]
      - "zscore": subtract mean, divide by std
      - "none":   plot raw values (no rescaling)
    """
    if construction_1 not in weighted_by_construction_1:
        raise KeyError(
            f"construction_1={construction_1!r} not found in weighted_by_construction_1; "
            f"available constructions: {sorted(weighted_by_construction_1.keys())}")
    if construction_2 not in weighted_by_construction_2:
        raise KeyError(
            f"construction_2={construction_2!r} not found in weighted_by_construction_2; "
            f"available constructions: {sorted(weighted_by_construction_2.keys())}")

    df1 = weighted_by_construction_1[construction_1]
    df2 = weighted_by_construction_2[construction_2]
    betas = [b for b in df1.columns if b in df2.columns]
    if not betas:
        raise ValueError(
            f"No shared beta values between {construction_1!r} (betas={list(df1.columns)}) "
            f"and {construction_2!r} (betas={list(df2.columns)}); they must come from sweeps "
            f"run over the same beta_values.")

    ncols = 2 if len(betas) > 1 else 1
    nrows = -(-len(betas) // ncols)  # ceil division
    fig, axes = plt.subplots(nrows, ncols, figsize=(5 * ncols, 4 * nrows), squeeze=False, sharey=True)
    flat_axes = axes.flatten()
    for ax, beta in zip(flat_axes, betas):
        ax.plot(df1.index, _normalize_series(df1[beta], normalize), marker="o", label=construction_1)
        ax.plot(df2.index, _normalize_series(df2[beta], normalize), marker="s", label=construction_2)
        ax.set_xlabel("sigma")
        ax.set_ylabel(f"normalized weighted convergence time")
        ax.set_title(f"beta={beta:.3g}")
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=8)
    for ax in flat_axes[len(betas):]:
        ax.set_visible(False)
    title = f"Shape comparison: {construction_1} vs {construction_2}"
    if hastle_name is not None:
        title += f"  (weighting: {hastle_name})"
    fig.suptitle(title)
    plt.tight_layout()
...
def refine_sigma_minima(
    ...
    for _, row in minima_df.iterrows():
        ...
        if metric == "inverse_gap":
            fine_weighted = spectral_gap_times_hastle(
...
def sweep_and_locate_sigma_minima(
    ...
    """End-to-end: a dense coarse sigma sweep -> weighted inverse spectral gap -> per-(construction,
    beta) sigma minimum -> refined minimum on a finer local grid around each one found.

    This is the same computation as `run_convergence_sweep` + `spectral_gap_times_hastle`,
    ...
    Returns a dict with keys:
      ...
      "coarse_weighted"    -- {construction: pivot(sigma, beta)}, same shape as
                               `spectral_gap_times_hastle`'s return value
    ...
    if metric == "inverse_gap":
        coarse_weighted = spectral_gap_times_hastle(
    ...
    refined_minima_df, refined_results = refine_sigma_minima(
        ...
        rho_init=rho_init, metric=metric, verbose=verbose)
...
def fit_c_over_beta(minima_df: pd.DataFrame, constructions: Sequence[str] = CONSTRUCTION_ORDER,
                     value_col: str = "refined_min_sigma") -> pd.DataFrame:
    """Least-squares fit of sigma_min(beta) = c / beta, separately for each construction.

    The model is linear in c once expressed in terms of x = 1/beta (y = c*x), so the least-squares
    solution has the closed form c = sum(x*y) / sum(x**2) (ordinary least squares through the
    origin) -- no iterative solver needed.

    Drops rows without an interior minimum first, if `minima_df` has a "has_interior_minimum"
    column (see `plot_sigma_minima_vs_beta`). Returns a DataFrame with one row per construction
    that has at least one data point: columns "construction", "c", "r_squared".
    """
    ...
    for construction in constructions:
        sub = minima_df[minima_df["construction"] == construction]
        if sub.empty:
            continue
        ...
        records.append({"construction": construction, "c": c, "r_squared": r_squared})
...
def plot_sigma_minima_vs_beta(minima_df: pd.DataFrame,
                               ...
                               show_inverse_beta: bool = False,
                               fit_inverse_beta: bool = True) -> None:
    ...
    If `fit_inverse_beta` is True (default), fits sigma_min(beta) = c/beta per construction (see
    `fit_c_over_beta`) and overlays each fitted curve -- dashed, same color as that construction's
    data -- with the fitted c and R^2 in the legend.

    If `show_inverse_beta` is True, additionally overlays the plain c=1 reference curve 1/beta.
    """
    if "has_interior_minimum" in minima_df.columns:
        minima_df = minima_df[minima_df["has_interior_minimum"]]
    ...
    fits = fit_c_over_beta(minima_df, constructions=present, value_col=value_col) if fit_inverse_beta else None

    for construction in present:
        sub = minima_df[minima_df["construction"] == construction].sort_values("beta")
        line, = ax.plot(sub["beta"], sub[value_col], marker="o", label=construction)

        if fit_inverse_beta:
            row = fits[fits["construction"] == construction]
            if not row.empty:
                c_fit, r_squared = row["c"].iloc[0], row["r_squared"].iloc[0]
                beta_fit = np.sort(sub["beta"].unique())
                ax.plot(beta_fit, c_fit / beta_fit, linestyle="--", color=line.get_color(),
                         label=f"{construction} fit: c={c_fit:.3g}/beta ($R^2$={r_squared:.3f})")

    if show_inverse_beta:
        betas = np.sort(minima_df["beta"].unique())
        ax.plot(betas, 1.0 / betas, linestyle=":", color="black", label="1/beta")
